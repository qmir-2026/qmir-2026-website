...
> Thanks to our **Quarto literate programming workflow**, you are living in the golden age of commenting -- you can literally write full prose between code chunks instead of apologizing in hashtags later.
...
*[Image suggestion: a clean two-column table or visual grid — type name on left, example on right, perhaps color-coded]*
...
| `factor` | Ordered categories | `"low"`, `"medium"`, `"high"` |
...
For most purposes in this course, the distinction doesn't matter — R handles it behind the scenes. But it's good to know it exists.
...
## Specialized Types: Factors, Dates, Times {.smaller}
...
*[Image suggestion: a layered or nested diagram — this is the most important visual in the deck. Suggest: scalar at top, then vector, then matrix (2D, one type) and list (flexible), then dataframe at bottom as the combination. Color-code "homogeneous" vs "heterogeneous".]*
...
population <- c(84, 68, 60, 47) # millions
...
- 2D, one type
- Useful for linear algebra behind the scenes
...
*[Image suggestion: a small visual table with column headers and colored cells — like a mini spreadsheet with country/pop/eu_member columns]*
...
eu_data <- data.frame(
  ...
  population = c(84, 68, 60),
  eu_member = c(TRUE, TRUE, TRUE),
  regime = c("democracy", "democracy", "democracy")
)
...
str(eu_data) # compact structure overview
```
...
## Indexing Data Frames with `[ ]` and `$` {.smaller}
...
country_info[["name"]] # double brackets for lists
country_info$population
country_info$parties[2] # chain indexing
```
...
| Syntax | Returns |
|---|---|
| `list["name"]` | A list of length 1 |
| `list[["name"]]` | The element itself |
| `list$name` | The element itself (same as `[[]]`) |
...
typeof(eu_data$population)
```

. . .

These are the first things you should run whenever you encounter a new dataset.
...
- Exploring a list: navigating model output structure
